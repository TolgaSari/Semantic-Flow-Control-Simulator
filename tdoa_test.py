...
if __name__ == '__main__':
    ...
    if config['test_no_filter']:
        test_cases.append({
            'name': 'No Filter',
            'filter': lambda: EWMA(np.zeros(state_size), 1)
        })

    if config['test_ewma']:
        test_cases.append({
            'name': 'EWMA',
            'filter': lambda: EWMA(np.zeros(state_size), 0.7)
        })
    ...
    if config['test_kalman']:
        test_cases.append({
            'name': 'Kalman',
            'filter': lambda: KalmanFilter(F_generator, period, B.copy(), H.copy(), x0.copy(), P0.copy(), R.copy(),
                                           acceleration_std_dev)
        })

    if config["test_semantic_kalman"]:
        ...
        def create_semantic_kalman_filter():
            filter_instance = SemanticKalmanFilter(F_generator, period, B.copy(), H.copy(), x0.copy(), P0.copy(), R.copy(), acceleration_std_dev)
    ...
    per_run_stats_df.to_csv(f'no_imu_stats_{int(period*1000)}.csv', index=False)
